...
# Center-crop to a square so landscape/portrait photos don't squash
w, h = im.size
side = min(w, h)
left = (w - side) // 2
top = max(0, (h - side) // 3)   # bias upward to keep face in frame
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">mahakaal2005@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'mahakaal2005@github:~$ whoami <tspan fill="{INK}">Atul Kumar Singh</tspan></text>')
